...
ONLINE_SAVE_INDEX = np.array([5, 10, 20, 30, 50, 100, 150, 200, 300, 400, 500, 600, 700, 800, 900, 1000], dtype=int)
OFFLINE_SAVE_INDEX = np.array([5, 10, 20, 30, 50, 100, 150, 200, 300, 400, 500, 600, 700, 800, 900, 1000], dtype=int)

SYSID_METHOD = 'STACKING'  # 'N4SID', 'LSTM', 'NNARX', 'STACKING'
ESTIMATION_METHOD = 'STACKING'  # 'KF', 'LSTM_KF', 'STACKING', 'RL_STACKING'
CONTROL_METHOD = 'SAC'  # 'QMPC', 'MPC', 'DDPG', 'TD3', 'SAC'
EXTRA_FILE_NAME = 'B512D9E5AL1CL10UP100ALPHA10'
...
def online_learning(config1, config2, file_name):
    ...
    #     controller1.controller.pre_update_critic_by_model(50)
    #     controller2.controller.pre_update_critic_by_model(50)

    """ Losses """
    actor_loss1 = np.zeros((60*ONLINE_HOUR + 1, 1), dtype=NP_DATA_TYPE)
    critic_loss1 = np.zeros((60*ONLINE_HOUR + 1, 1), dtype=NP_DATA_TYPE)
    actor_loss2 = np.zeros((60*ONLINE_HOUR + 1, 1), dtype=NP_DATA_TYPE)
    critic_loss2 = np.zeros((60*ONLINE_HOUR + 1, 1), dtype=NP_DATA_TYPE)
    ...
    for k in range(60*ONLINE_HOUR):
        ...
        # Controller learning
        if np.mod(np.floor(k/60), 2) == 0:
            controller1.save_data_to_buffer(x_est[k, :], u[k, :], c[k, :], x_est[k + 1, :], False)
            actor_loss1[k, :], critic_loss1[k, :] = controller1.train()
        else:
            controller2.save_data_to_buffer(x_est[k, :], u[k, :], c[k, :], x_est[k + 1, :], False)
            actor_loss2[k, :], critic_loss2[k, :] = controller2.train()
        ...
        if int(k + 1) in 60*ONLINE_SAVE_INDEX:
            ...
            if SAVE_BOOL:
                ...
                config1.save_something(directory, file_name, 'actor-loss1', actor_loss1)
                config1.save_something(directory, file_name, 'critic-loss1', critic_loss1)
                config1.save_something(directory, file_name, 'actor-loss2', actor_loss2)
                config1.save_something(directory, file_name, 'critic-loss2', critic_loss2)
