...
"""
import numpy as np
from Sysids import sysid_n4sid
from Sysids import sysid_lstm
from Sysids import sysid_nnarx
...
class SYSID(object):
    def __init__(self, plant, config):
        ...
        self.ss_transform = config.SYSID_ss_transform if config.SYSID_ss_transform is not None else True
    ...
    def add_data_and_scale(self, u, y):
        ...
        if self.sysid_method in ['N4SID', 'LSTM', 'NNARX']:
            self.sysid.add_data_and_scale(self.scaled_u, self.u_bias, self.u_scale,
                                          self.scaled_y, self.y_bias, self.y_scale)
        else:
            pass
